...
canvas = scene.SceneCanvas(title='Torus', bgcolor='#050510', size=(900, 600), show=True)
view = canvas.central_widget.add_view()
...
R, r = 1.5, 0.5  # major radius, minor radius
...
colors = np.zeros((n, 4))
colors[:, 0] = np.abs(np.sin(u))
colors[:, 1] = np.abs(np.sin(u + 2.1))
colors[:, 2] = np.abs(np.sin(u + 4.2))
colors[:, 3] = 0.9

torus = scene.visuals.Markers(pos=pts, size=2, face_color=colors,
                               edge_width=0, parent=view.scene)

def on_timer(event):
    torus.transform = scene.transforms.MatrixTransform()
    torus.transform.rotate(event.elapsed * 25, (0, 0, 1))
    canvas.update()
